    ...
    def aggregate_order(self, customer: str, max_items_quantity: int, max_volume: int):
        ...
        for i in self.order_items.copy():
            if i.customer == customer:
                if qu + i.quantity <= max_items_quantity and vol + i.total_volume <= max_volume:
                    items.append(i)
                    vol += i.total_volume
                    qu += i.quantity
...
class ContainerAggregator:
    ...
    def prepare_containers(self, orders: tuple) -> dict:
        ...
        curdes = None
        for i in orders:
            if i.destination not in orderdict and vol + i.total_volume <= self.container_volume:
                orderdict[i.destination] = []
                curdes = i.destination
        use = []
        use2 = []
        use3 = []
        for i in orders:
            if vol + i.total_volume <= self.container_volume and i not in self.not_used_orders:
                vol += i.total_volume
                use.append(i)
                print(f"appended {i.destination}")
            elif vol + i.total_volume >= self.container_volume and i.destination == curdes:
                vol2 += i.total_volume
                use2.append(i)
            elif vol2 + i.total_volume >= self.container_volume and i.destination == curdes:
                vol3 += i.total_volume
                use3.append(i)
            else:
        ...
        print(len(use))
        for i in use:
            contain = Container(self.container_volume, tuple(use))
            contain.orders.append(i)
            orderdict[i.destination].append(contain)
        for i in use2:
            contain = Container(self.container_volume, tuple(use2))
            contain.orders.append(i)
            orderdict[i.destination].append(contain)
        for i in use3:
            contain = Container(self.container_volume, tuple(use3))
            contain.orders.append(i)
            orderdict[i.destination].append(contain)
        return orderdict
